# TheHER — Hydrogen Evolution Reaction Fitting Webapp

This project provides a lightweight Flask webapp to upload LSV-like data, fit HER kinetic models (simplified and full), and render plots and derived analyses (coverage and Tafel slope).
...
- Flask MVC scaffold in `webapp/` (models, services, routes, templates)
- HER model and fitting in `webapp/models/hydrogen.py`
- Service layer in `webapp/services/fitting_service.py`
- Main UI and summary templates in `webapp/templates/`
- Unit tests in `tests/test_hydrogen.py`
- Example fixture `test_fixture.csv`

If you want a production deployment I can add Gunicorn config, a Dockerfile, or orchestrate deployment to a PaaS.

---
Generated by helper scripts in this workspace.
